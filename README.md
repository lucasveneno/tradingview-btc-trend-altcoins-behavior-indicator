# BTC & Altcoins Trend Indicator 

![TradingView](https://img.shields.io/badge/Platform-TradingView-blue)
![Pine Script](https://img.shields.io/badge/Language-Pine%20Script%20v6-orange)
![License](https://img.shields.io/badge/License-MPL%202.0-brightgreen)

A **TradingView Pine Script** indicator to analyze **Bitcoin trends** and their impact on **altcoins** across multiple timeframes and exchanges.

---

## Screenshot 

![BTC & Altcoins Trend Indicator](screenshot.png)

---

## Features 

- **BTC Trend Detection**
  - Mode 1: Price-based
  - Mode 2: EMA-based
- **Multi-Timeframe Support**
  - 1 min, 15 min, 1h, 4h, 1d, 1w, 1M
- **Altcoins Behavior Analysis**
  - Visualizes altcoins performance relative to BTC
- **Customizable Moving Averages**
  - Up to 4 EMAs + 1 SMA with adjustable values/colors
- **Summary Table**
  - Quick overview of trends and altcoin behavior
- **Visual Trend Lines**
  - Extend EMAs directly on chart

---

## Supported Exchanges 

| Exchange | Symbol Example |
|----------|----------------|
| Binance  | BTCUSDT        |
| Bybit    | BTCUSD         |
| Coinbase | BTCUSD         |
| Kraken   | XBTUSD         |

---

## Installation 

1. Open [TradingView](https://www.tradingview.com/).
2. Go to **Pine Editor**.
3. Copy and paste the script code.
4. Click **Add to Chart**.

Alternatively, try the script directly on TradingView:  
[Try the Indicator on TradingView](https://www.tradingview.com/script/jgwMdlN7/)

---

## Usage 

- Monitor BTC trend across multiple timeframes.
- Track altcoins relative to BTC dominance.
- Customize EMAs/SMA for personal strategy.

---

## Contributing 

Open to **pull requests** and **issues**. Suggestions for new features and improvements are welcome.

---

## License 

**Mozilla Public License 2.0 (MPL 2.0)**  
[Read more](https://mozilla.org/MPL/2.0/)

---

## Disclaimer

This indicator is **educational only** and **not financial advice**. Cryptocurrency trading involves **high risk**. Always perform your own research.
